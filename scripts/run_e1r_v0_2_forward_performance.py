#!/usr/bin/env python3
"""
E1R v0.2 forward performance runner with KICKOFF_READY preservation guard.

This wrapper protects accepted E1R paper orders / positions before LIVE_FORWARD starts.

Guard behavior:
- If tracking_status == KICKOFF_READY
- and official_kickoff_date is null
- and accepted paper orders / positions already exist
then the underlying performance script may update equity/performance scaffolding,
but it must not erase official E1R paper orders / positions or prematurely set
official_kickoff_date.

The original implementation is kept in:
...
CORE_SCRIPT = ROOT / "scripts" / "run_e1r_v0_2_forward_performance_core.py"
...
def count_positions(doc: Any) -> int:
    if isinstance(doc, dict) and isinstance(doc.get("positions"), list):
        return len(doc["positions"])
    if isinstance(doc, list):
        return len(doc)
    return 0


def get_positions(doc: Any) -> list:
    if isinstance(doc, dict) and isinstance(doc.get("positions"), list):
...
def should_preserve(before: Dict[str, Any]) -> bool:
    ...
    orders_doc = before["orders_doc"]
    positions_doc = before["positions_doc"]
    state = before["state"]
    ...
    return (
        ...
        and count_orders(orders_doc) > 0
        and count_positions(positions_doc) > 0
...
def restore_kickoff_ready_paper_state(before: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "preserved": True,
        "orders_count": count_orders(before_orders),
        "positions_count": count_positions(before_positions),
...
def main() -> int:
    ...
    if preserve:
        result = restore_kickoff_ready_paper_state(before)
        print("E1R forward performance preservation guard applied")
        ...
        print("E1R forward performance preservation guard not active")
