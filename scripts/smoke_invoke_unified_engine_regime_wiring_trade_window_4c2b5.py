...
REPORT_JSON = ROOT / "docs/research/E1R_UNIFIED_5Y_FULL_ACCOUNT_V1_4C2B5_REGIME_WIRING_TRADE_WINDOW_REPORT.json"
REPORT_MD = ROOT / "docs/research/E1R_UNIFIED_5Y_FULL_ACCOUNT_V1_4C2B5_REGIME_WIRING_TRADE_WINDOW_REPORT.md"
...
def build_assumptions(required_keys: list[str]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    assumptions["e1r_shell_mode"] = "regime_wiring_trade_window_smoke"
    assumptions["e1r_uptrend_execution_enabled"] = True
    assumptions["strategy_variant"] = "E1R_UNIFIED_5Y_FULL_ACCOUNT_V1_REGIME_WIRING_TRADE_WINDOW_SMOKE"
    assumptions["version"] = "4C-2B-5-regime-wiring-trade-window-smoke"
...
def build_inputs(smoke_symbols_limit: int = 12) -> dict[str, Any]:
    ...
    # This is the previous known trade-capable smoke window from 4C-2B-3.
    sim_start_date = "2021-04-14" if "2021-04-14" in set(spx_dates) else (spx_dates[260] if len(spx_dates) > 360 else None)
    sim_end_date = "2021-07-09" if "2021-07-09" in set(spx_dates) else (spx_dates[min(320, len(spx_dates)-1)] if len(spx_dates) > 360 else None)
...
def summarize_daily_regime_output(result: Any) -> dict[str, Any]:
    records = []
    if isinstance(result, dict):
    ...
    for r in records:
        regime_counts[r.get("spx_regime")] += 1
        active_mode_counts[r.get("e1r_active_mode")] += 1
        subclass_counts[r.get("sideways_subclass") or r.get("subclass")] += 1
    ...
    return {
        "daily_records_source_length": len(records),
        "non_null_regime_count": non_null_regime_count,
        "non_null_active_mode_count": non_null_active_mode_count,
        "regime_counts": {str(k): v for k, v in regime_counts.items()},
        "active_mode_counts": {str(k): v for k, v in active_mode_counts.items()},
        "subclass_counts": {str(k): v for k, v in subclass_counts.items()},
        "first": records[0] if records else None,
        "last": records[-1] if records else None,
        "sample_non_null_regime": next((r for r in records if r.get("spx_regime") is not None), None),
        "sample_non_null_active_mode": next((r for r in records if r.get("e1r_active_mode") is not None), None),
    }
...
def main() -> int:
    ...
    try:
        ...
        smoke["daily_regime_output_summary"] = summarize_daily_regime_output(result)
    ...
    regime_wired_observed = daily_out.get("non_null_regime_count", 0) > 0
    active_mode_observed = daily_out.get("non_null_active_mode_count", 0) > 0

    if import_probe["ok"] and smoke["ok"] and has_daily_records and regime_wired_observed and active_mode_observed:
        conclusion = "STATEFUL_ENGINE_REGIME_WIRING_TRADE_WINDOW_OK_READY_FOR_FULL_5Y_UNIFIED_RUN"
        recommended = "Proceed to 4C-2C: run full 5Y unified full-account backtest with regime wiring enabled."
    elif import_probe["ok"] and smoke["ok"] and has_daily_records and regime_wired_observed:
        conclusion = "STATEFUL_ENGINE_REGIME_WIRING_PARTIAL_ACTIVE_MODE_NOT_OBSERVED"
        recommended = "Full run may proceed only if active_mode is diagnostic; otherwise map active_mode contract before official run."
    elif import_probe["ok"] and smoke["ok"] and has_daily_records:
        conclusion = "STATEFUL_ENGINE_HAS_DAILY_RECORDS_BUT_REGIME_FIELDS_NOT_WIRED"
        recommended = "Inspect backtest.py regime lookup contract; do not run official 5Y until regime fields appear in daily records."
    elif import_probe["ok"] and smoke["ok"]:
        conclusion = "STATEFUL_ENGINE_TRADE_WINDOW_SMOKE_OK_BUT_OUTPUT_CONTRACT_NEEDS_MAPPING"
        recommended = "Map returned object fields to daily equity contract before full 5Y."
    elif import_probe["ok"]:
        conclusion = "PACKAGE_IMPORT_OK_TRADE_WINDOW_SMOKE_FAILED"
        recommended = "Use traceback to fix smoke input contract, then retry."
    else:
    ...
    report = {
        "generated_at": now(),
        "stage": "E1R_UNIFIED_5Y_FULL_ACCOUNT_V1_4C2B5_REGIME_WIRING_TRADE_WINDOW",
        "status": "E1R_UNIFIED_ENGINE_REGIME_WIRING_TRADE_WINDOW_COMPLETE_NO_FULL_BACKTEST",
        "policy": {
        ...
        "active_mode_observed": active_mode_observed,
    ...
    md = []
    md.append("# E1R Unified 5Y Full Account V1 — 4C-2B-5 Regime Wiring Trade Window")
    md.append("")
    ...
    md.append("")
    md.append("```json")
    md.append(json.dumps(smoke, indent=2, ensure_ascii=False)[:42000])
    md.append("```")
    md.append("")
    ...
    print("Stage 4C-2B-5 regime wiring trade window complete")
    ...
    print("daily_regime_output_summary:", json.dumps(smoke.get("daily_regime_output_summary"), ensure_ascii=False)[:12000])
    ...
    print("active_mode_observed:", active_mode_observed)
