...
REPORT_JSON = ROOT / "docs/research/E1R_UNIFIED_5Y_FULL_ACCOUNT_V1_4C2B2_SMOKE_REAL_ASSUMPTIONS_REPORT.json"
REPORT_MD = ROOT / "docs/research/E1R_UNIFIED_5Y_FULL_ACCOUNT_V1_4C2B2_SMOKE_REAL_ASSUMPTIONS_REPORT.md"
...
ASSUMPTION_SOURCE_CANDIDATES = [
    ...
    ROOT / "docs/research/E1R_UNIFIED_5Y_FULL_ACCOUNT_V1_4C2B1_SMOKE_RETRY_REPORT.json",
]
...
# Conservative, numeric-safe defaults matching existing Layer D / E1 style names.
# These are smoke defaults only; 4C-2C full run will freeze the final assumption contract.
HARD_DEFAULTS = {
...
def build_assumptions(required_keys: list[str]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for k in required_keys:
        if k in rec["values"] and rec["values"][k] is not None:
            ...
            # Last resort: keep type-safe and non-None when key is used by subscript.
            lk = k.lower()
    ...
    # Add optional helpful defaults even if not used directly.
...
def main() -> int:
    ...
    report = {
        "generated_at": now(),
        "stage": "E1R_UNIFIED_5Y_FULL_ACCOUNT_V1_4C2B2_SMOKE_REAL_ASSUMPTIONS",
    ...
    md = []
    md.append("# E1R Unified 5Y Full Account V1 — 4C-2B-2 Smoke Real Assumptions")
    md.append("")
    md.append(f"Generated At: `{report['generated_at']}`")
    md.append("")
    md.append("## Status")
    md.append("")
    md.append("- Status: `E1R_UNIFIED_ENGINE_SMOKE_REAL_ASSUMPTIONS_COMPLETE_NO_FULL_BACKTEST`")
    ...
    md.append("")
    md.append("```json")
    md.append(json.dumps(smoke, indent=2, ensure_ascii=False)[:36000])
    md.append("```")
    md.append("")
    ...
    print("Stage 4C-2B-2 smoke real assumptions complete")
    ...
    print("result_summary:", json.dumps(smoke.get("result_summary"), ensure_ascii=False)[:10000])
