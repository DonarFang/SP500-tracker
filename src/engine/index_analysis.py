...
def analyze_index(code: str, prices: list[float], dates: list[str]) -> dict:
    n = len(prices)
    if n < 5: return {"code":code,"available":False}
    ...
    r = {"code":code,"yahoo_symbol":INDEX_SYMBOLS.get(code,code),"name":INDEX_META[code]["name"],
         "desc":INDEX_META[code]["desc"],"available":True,
         "price":round(last,2),"change":round(chg,2),"change_pct":round(chg_pct,2),
         "ma20":round(ma20,2),"ma50":round(ma50,2),"ma200":round(ma200,2),
         "above_ma20":last>ma20,"above_ma50":last>ma50,"above_ma200":last>ma200,
         "slope20":round(sl20,6),"high52w":round(high52,2),"low52w":round(low52,2),
         "pct_from_high":round(pct_from_high,2),
         "chart_dates":dates[-60:],"chart_prices":[round(p,2) for p in prices[-60:]],
         "chart_ma20":chart_ma20,"chart_ma50":chart_ma50}
    if code=="VIX":
        if last<15:   st,sig,col="低恐惧","Risk-On","#1D9E75"
        elif last<20: st,sig,col="正常","Neutral","#378ADD"
        elif last<25: st,sig,col="偏高","Caution","#BA7517"
        elif last<35: st,sig,col="高恐惧","Risk-Off","#D85A30"
        else:         st,sig,col="极度恐惧","Extreme Fear","#993C1D"
        trend = "上升" if sl20>0.001 else "下降" if sl20<-0.001 else "横盘"
        r.update({"vix_state":st,"vix_signal":sig,"vix_color":col,"vix_trend":trend})
    else:
...
def analyze_all_indices(prices_map: dict, dates_map: dict) -> dict:
    ...
    for code, sym in INDEX_SYMBOLS.items():
        p = prices_map.get(sym) or prices_map.get(sym.replace("^","_"),[])
        d = dates_map.get(sym)  or dates_map.get(sym.replace("^","_"),[])
        results[code] = analyze_index(code,p,d) if len(p)>=5 else {"code":code,"name":INDEX_META[code]["name"],"available":False}
    # 相对强弱
    ndx_p = prices_map.get("^NDX") or prices_map.get("_NDX",[])
    spx_p = prices_map.get("^GSPC") or prices_map.get("_GSPC",[])
    sox_p = prices_map.get("^SOX") or prices_map.get("_SOX",[])
