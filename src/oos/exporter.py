...
def export_all(state, events: list, manifest: dict) -> None:
    ts = _now()
    try:
        disp = _now_display()
    except Exception:
    ...
    write_json("oos_summary.json", {
        ...
        "note": "OOS log is append-only. BACKFILLED_PRELIVE and LIVE_FORWARD are displayed separately.",
    })
        ...
        "positions": [
            {
                ...
                "source":       h.get("source", "LIVE_FORWARD"),
            }
    ...
    write_json("oos_orders.json", {
        "generated_at": ts, "generated_at_display": disp,
        "strategy_id": strat_id,
        "orders": order_events[-100:],  # last 100
    })
